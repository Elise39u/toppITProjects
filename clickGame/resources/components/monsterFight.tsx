...
import { getScalingForLevel } from '../../config/gameRules';
...
const MonsterFight: React.FC<MonsterObj> = ({ Monster, user, Area }) => {
    ...
    function HandleHPCheck(checkedOn: string) {
        if (checkedOn === "player") {
            if (tempUserHP <= 0) {
    ...
    const handleAction = async (actionType: string) => {
        ...
        switch (actionType) {
            case "Fight":
                ...
                HandleHPCheck("monster");
                await sleep(500);
                await MonsterAI();
                HandleHPCheck("player");
                break;
            case "Taunt":
    ...
    function handleFightNoRewards() {
        router.post('/user/winfight/' + user.id, {
            player_id: user.id,
            curhp: tempUserHP,
            xp: user.current_exp,
            gold: user.gold
            ...
            onSuccess: async () => {
                addLog(`You have fled the fight no rewarsd given. HP updated.`)
                await sleep(2000);
            },
            onError: async () => {
                await sleep(2000);
    ...
    function handleAfterFight() {
        if (tempUserHP <= 0) {
            ...
            let newUserXP = user.current_exp + Monster.xp;

            const scaling = getScalingForLevel(user.level);
            if (newUserXP >= user.exp_to_next_level) {
                let nextLvLXP = user.exp_to_next_level + 250 + scaling.xpAmount * Math.pow(user.level, 2);
                let newuserLevel = user.level + 1;
                let rewardGold = Monster.gold + 250 * Math.pow(user.level, 2);
                ...
                router.post('/user/winfight/' + user.id, {
                    ...
                    level: newuserLevel,
                    exp_to_next_level: nextLvLXP
                }, {
                    onSuccess: async () => {
                        addLog(`You have leveled up to level ${newuserLevel}! You gained ${rewardGold} gold and ${Monster.xp} XP!`)
                        await sleep(2000);
                    },
                    onError: async () => {
                        await sleep(2000);
                ...
                router.post('/user/winfight/' + user.id, {
                    ...
                    curhp: tempUserHP,
                    xp: newUserXP,
                    gold: user.gold + Monster.gold,
                }, {
                    onSuccess: async () => {
                        addLog(`Rewards given. You earned ${Monster.gold} gold, and xp ${Monster.xp}`)
                        await sleep(2000);
                    },
                    onError: async () => {
                        await sleep(2000);
